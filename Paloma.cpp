...
float tiempoOffset;
float retardo;
float Paloma_TX = 0.0f;
float Paloma_TY = 0.0f;
float Paloma_TZ = 0.0f;
float Paloma_RX = 0.0f;
float Paloma_RY = 0.0f;
float Paloma_RZ = 0.0f;
...
int main()
{
	...
	Paloma = Model();
	Paloma.LoadModel("Models/Paloma.obj");
	Bicicleta = Model();
	...
	while (!mainWindow.getShouldClose())
	{
		...
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(0.0f + Paloma_TX, 20.0f + Paloma_TY, 0.0f + Paloma_TZ));
		model = glm::rotate(model, (-90.0f + Paloma_RX)* toRadians, glm::vec3(1.0f, 0.0f, 0.0f));
		model = glm::rotate(model, (90.0f + Paloma_RY)* toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
		model = glm::rotate(model, (90.0f + Paloma_RZ)* toRadians, glm::vec3(0.0f, 0.0f, 1.0f));
		...
		model = glm::rotate(model, (-90.0f + Bicicleta_RX)* toRadians, glm::vec3(1.0f, 0.0f, 0.0f));
		model = glm::rotate(model, (90.0f + Bicicleta_RY)* toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
		model = glm::rotate(model, (90.0f + Bicicleta_RZ)* toRadians, glm::vec3(0.0f, 0.0f, 1.0f));
		...
		//Paloma
		//Avanza
		if (Estado_Paloma == 0 && Paloma_TX < 40.0f) {
			Paloma_TX += 0.1;
		}
		//Gira
		if (Paloma_TX >= 40.0f) {
		...
		if (Estado_Paloma == 3 && Paloma_TX > 30.0f) {
			if (Paloma_RZ <= 0.0) {
				Paloma_RZ += 3;
				Paloma_TZ -= 0.3;
				Paloma_TX += 0.3;
			}
			if (Paloma_RZ > 0.0 && Paloma_RZ < 90.0) {
				Paloma_RZ += 3;
				Paloma_TZ -= 0.3;
				Paloma_TX -= 0.3;
			}
		}
		if (Estado_Paloma == 3 && Paloma_RZ >= 90.0f) {
			Estado_Paloma = 4;
			Paloma_RZ = 90;
		}
		//Retrocede
		if (Estado_Paloma == 4 && (Paloma_TX > -8.5f)) {
			Paloma_TX -= 0.1;
		}
		if (Paloma_TX <= -8.5f) {
			Estado_Paloma = 5;
		...
		//Gira otra vez
		if (Estado_Paloma == 5 && Paloma_RZ < 270.0f) {
			if (Paloma_RZ >= 0.0) {
				Paloma_RZ += 5;
				Paloma_TZ += 0.3;
				Paloma_TX -= 0.3;
			}
			if (Paloma_RZ < 0.0 && Paloma_RZ > 180.0) {
				Paloma_RZ += 5;
				Paloma_TZ -= 0.3;
				Paloma_TX += 0.3;
			}
